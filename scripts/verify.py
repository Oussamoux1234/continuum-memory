...
def packaging_smoke() -> None:
    with tempfile.TemporaryDirectory(prefix="continuum-package-", dir=str(ROOT / "work")) as temp:
        ...
        run([python, "-m", "pip", "install", "--no-deps", "-e", str(ROOT)])
        run([str(Path(environment) / "bin" / "continuum"), "--version"])
        run([str(Path(environment) / "bin" / "memoryd"), "--help"])
        run([str(Path(environment) / "bin" / "continuum-mcp"), "--help"])
