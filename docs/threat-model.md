...
| Supply-chain/network | no default runtime network code/telemetry; Linux installer isolates PATH and Python/pip environment; Issue #13 uses one digest-pinned manylinux environment to acquire and verify signed SQLCipher/OpenSSL sources, revalidates exact source/tool/recipe hashes, runs two network-disabled read-only builds per CPython 3.11-3.14 target, compares exact wheels, inspects their complete inventory/native hardening, and performs isolated offline runtime tests before success-only retention | The application runtime remains plaintext and does not consume these test artifacts; GitHub runner kernel/engine/CPU and registry/service availability are not pinned; macOS is blocked without an approved immutable builder; Windows is unsupported; binding license remains `NOASSERTION`; signing identity, independent acceptance, migration/rollback evidence, and permanent distribution are absent |
