...
| Supply-chain/network | no default runtime network code/telemetry; Linux installer isolates PATH and Python/pip environment; Issue #13 pins signed SQLCipher/OpenSSL sources, exact build wheels, and an immutable manylinux builder, then disables build/test networking and compares two native artifacts | The current runtime remains plaintext on main; GitHub runner kernel/engine/CPU are not pinned; macOS and other Python ABIs are not yet built; binding license is `NOASSERTION`; artifact signing identity and permanent distribution are deliberately absent |
