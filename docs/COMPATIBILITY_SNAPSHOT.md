...
Recorded 2026-09-05 for design; runtime and builds make no network requests.
...
| SQLCipher | https://www.zetetic.net/sqlcipher/sqlcipher-api/ | Issue #7 candidate pins `sqlcipher3` 0.6.2 / SQLCipher 4.12.0, raw 32-byte key, encrypted WAL, integrity check, eight-wheel component/license inventory and conservative SPDX record; no patched published binding wheel exists, and known OpenSSL 3.6.0 / SQLite 3.51.1 findings plus license/signing/supply-chain decisions block acceptance |
...
request; acquiring the hash-pinned SQLCipher wheel is a separate installation step. This
version snapshot is evidence for the candidate, not a release recommendation; see
`RELEASE_READINESS.md` for the exact blocked audit.
